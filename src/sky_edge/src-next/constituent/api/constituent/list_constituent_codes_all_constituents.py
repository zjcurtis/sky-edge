...
from ...models.api_collection_of_constituent_code_read import ApiCollectionOfConstituentCodeRead
...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/constituents/constituentcodes",
        "params": params,
    }
...
) -> Any | ApiCollectionOfConstituentCodeRead | None:
    if response.status_code == 200:
        response_200 = ApiCollectionOfConstituentCodeRead.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | ApiCollectionOfConstituentCodeRead]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Constituent code list (All constituents)
     ...
     Returns a paginated list of constituent codes.
    <p />
    The default sorting behavior is to list constituent codes in ascending order based on when they were
    created.
    <p />
    However, the <code>last_modified</code> parameter overrides the default sorting behavior to list
    constituent codes in ascending order based on when they were last modified. The
    <code>last_modified</code> and <code>date_added</code> parameters also add the
    <code>sort_token</code> parameter to the <code>next_link</code> URL to ensure that constituent codes
    ...
    sorting behavior is to list constituent codes based on when they were last modified.
...
def sync(
    *,
    ...
    Returns:
        Any | ApiCollectionOfConstituentCodeRead
    """
