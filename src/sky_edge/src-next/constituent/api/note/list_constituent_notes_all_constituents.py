...
from ...models.api_collection_of_note_read import ApiCollectionOfNoteRead
...
) -> Any | ApiCollectionOfNoteRead | None:
    if response.status_code == 200:
        response_200 = ApiCollectionOfNoteRead.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | ApiCollectionOfNoteRead]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Note list (All constituents)
     ...
     Returns a paginated list of notes, excluding RE NXT Summary notes.
    <p />
    The default sorting behavior is to list notes in ascending order based on when they were created.
    <p />
    However, the <code>last_modified</code> parameter overrides the default sorting behavior to list
    notes in ascending order based on when they were last modified. The <code>last_modified</code>
    parameter also adds the <code>sort_token</code> parameter to the <code>next_link</code> URL to
    ensure that notes are stably sorted and that order persists when changes occur while working through
    a paginated list.
    <p />
    If the <code>last_modified</code> and <code>date_added</code> parameters are both specified, the
    sorting behavior is to list notes based on when they were last modified.
    <p />
