...
from ...models.api_collection_of_email_address_read import ApiCollectionOfEmailAddressRead
...
) -> Any | ApiCollectionOfEmailAddressRead | None:
    if response.status_code == 200:
        response_200 = ApiCollectionOfEmailAddressRead.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | ApiCollectionOfEmailAddressRead]:
    return Response(
...
def sync_detailed(
    *,
    ...
    <p />
    The default sorting behavior is to list email addresses in ascending order based on when they were
    created.
    <p />
    However, the <code>last_modified</code> parameter overrides the default sorting behavior to list
    email addresses in ascending order based on when they were last modified. The
    <code>last_modified</code> parameter also adds the <code>sort_token</code> parameter to the
    <code>next_link</code> URL to ensure that email addresses are stably sorted and that order persists
    ...
    sorting behavior is to list email addresses based on when they were last modified.
...
def sync(
    *,
    ...
    Returns:
        Any | ApiCollectionOfEmailAddressRead
    """
