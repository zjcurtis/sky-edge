...
from ...models.api_collection_of_address_read import ApiCollectionOfAddressRead
...
) -> Any | ApiCollectionOfAddressRead | None:
    if response.status_code == 200:
        response_200 = ApiCollectionOfAddressRead.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | ApiCollectionOfAddressRead]:
    return Response(
...
def sync_detailed(
    *,
    ...
    <p />
    The default sorting behavior is to list addresses in ascending order based on when they were
    created.
    <p />
    However, the <code>last_modified</code> parameter overrides the default sorting behavior to list
    addresses in ascending order based on when they were last modified. The <code>last_modified</code>
    and <code>date_added</code> parameters also add the <code>sort_token</code> parameter to the
    <code>next_link</code> URL to ensure that addresses are stably sorted and that order persists when
    ...
    sorting behavior is to list addresses based on when they were last modified.
...
def sync(
    *,
    ...
    Returns:
        Any | ApiCollectionOfAddressRead
    """
